...
namespace Any.Core.Rss
{
    ...
    public partial class RssFeed
    {
        ...
        {
            Title = new XElement(AnyRssDefaults.Title, title);
            Description = new XElement(AnyRssDefaults.Description, description);
            Link = new XElement(AnyRssDefaults.Link, link);
            LastBuildDate = new XElement(AnyRssDefaults.LastBuildDate, lastBuildDate.ToString("r"));
        }
        ...
        public static async Task<RssFeed> LoadAsync(Stream stream)
        {
            try
            {
                ...
                var channel = document.Root?.Element(AnyRssDefaults.Channel);

                if (channel == null)
                    return null;

                var title = channel.Element(AnyRssDefaults.Title)?.Value ?? string.Empty;
                var description = channel.Element(AnyRssDefaults.Description)?.Value ?? string.Empty;
                var link = new Uri(channel.Element(AnyRssDefaults.Link)?.Value ?? string.Empty);
                var lastBuildDateValue = channel.Element(AnyRssDefaults.LastBuildDate)?.Value;
                ...
                foreach (var item in channel.Elements(AnyRssDefaults.Item))
                {
        ...
        public string GetContent()
        {
            ...
            var root = new XElement(AnyRssDefaults.RSS, new XAttribute("version", "2.0"));
            var channel = new XElement(AnyRssDefaults.Channel,
