...
namespace Any.Core.Rss
{
    ...
    public partial class RssItem
    {
        ...
        public RssItem(string title, string content, Uri link, string id, DateTimeOffset pubDate)
        {
            ...
            Content = new XElement(AnyRssDefaults.Description, content);
            Link = new XElement(AnyRssDefaults.Link, link);
            Id = new XElement(AnyRssDefaults.Guid, new XAttribute("isPermaLink", false), id);
            PubDate = new XElement(AnyRssDefaults.PubDate, pubDate.ToString("r"));
        }
        ...
        {
            var title = item.Element(AnyRssDefaults.Title)?.Value ?? string.Empty;
            var content = item.Element(AnyRssDefaults.Content)?.Value ?? string.Empty;
            if (string.IsNullOrEmpty(content))
                content = item.Element(AnyRssDefaults.Description)?.Value ?? string.Empty;
            var link = new Uri(item.Element(AnyRssDefaults.Link)?.Value ?? string.Empty);
            var pubDateValue = item.Element(AnyRssDefaults.PubDate)?.Value;
            var pubDate = pubDateValue == null ? DateTimeOffset.Now : DateTimeOffset.ParseExact(pubDateValue, "r", null);
            var id = item.Element(AnyRssDefaults.Guid)?.Value ?? string.Empty;
        ...
        {
            var element = new XElement(AnyRssDefaults.Item, Id, Link, Title, Content);
