...
namespace Any.Core
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        private readonly IAnyFileProvider _fileProvider;
        ...
        protected virtual bool TryWriteWebConfig()
        {
            try
            {
                _fileProvider.SetLastWriteTimeUtc(_fileProvider.MapPath(AnyInfrastructureDefaults.WebConfigPath), DateTime.UtcNow);
                return true;
            }
            catch
            {
        ...
        public virtual string GetCurrentIpAddress()
        {
            ...
            try
            {
                ...
                if (_httpContextAccessor.HttpContext.Request.Headers != null)
                {
                    ...
                    var forwardedHttpHeaderKey = AnyHttpDefaults.XForwardedForHeader;
        ...
        public virtual bool IsCurrentConnectionSecured()
        {
            ...
            if (_hostingConfig.UseHttpClusterHttps)
                return _httpContextAccessor.HttpContext.Request.Headers[AnyHttpDefaults.HttpClusterHttpsHeader].ToString().Equals("on", StringComparison.OrdinalIgnoreCase);

            //use HTTP_X_FORWARDED_PROTO?
            if (_hostingConfig.UseHttpXForwardedProto)
                return _httpContextAccessor.HttpContext.Request.Headers[AnyHttpDefaults.HttpXForwardedProtoHeader].ToString().Equals("https", StringComparison.OrdinalIgnoreCase);
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false)
        {
            ...
            if (!success)
            {
                throw new AnyException("nopCommerce needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContextAccessor.HttpContext.Items[AnyHttpDefaults.IsPostBeingDoneRequestItem] == null)
                    return false;

                return Convert.ToBoolean(_httpContextAccessor.HttpContext.Items[AnyHttpDefaults.IsPostBeingDoneRequestItem]);
            }

            set => _httpContextAccessor.HttpContext.Items[AnyHttpDefaults.IsPostBeingDoneRequestItem] = value;
        }
